...
# -- CUSTOM_WIDGET_IMPORT -- #
# from Import_Fbx_Tools.Controls import My_Custom_QListWidget as My_Custom_QListWidget
# reload(My_Custom_QListWidget)
...
LOCAL_JSON = r'C:/Users/' + username + '/Documents/__GE_Tool_Prefs'
...
class Import_Fbx(QtWidgets.QMainWindow):
    ...
    def my_ui(self):
        ...
        # self.main_tab = QtWidgets.QTabWidget(self.centralwidgets)
        ...
        self.my_update_folder_button.setText('Select Folder')
    ...
    def call_back(self):
        try:
            # self.setting_functions(mode='folder_tab_change')
        ...
        self.my_update_objects_button.clicked.connect(partial(self.setting_functions, mode="update_liberary"))
    ...
    def update_listWidget(self):
        folder_name = Ultilities.detach_current_ui(self.project_tab, JSON_PATH)[2]
        SOURCE_PATH = Ultilities.detach_current_ui(self.project_tab, JSON_PATH)[0]
    ...
    def search_change(self):
        current_listwidget = Ultilities.detach_widget(self.project_tab)[2] #Current listWidget
        SOURCE_PATH = Ultilities.detach_current_ui(self.project_tab, JSON_PATH)[0]  # Source path
        name_item_selected = Ultilities.detach_current_ui(self.project_tab, JSON_PATH)[1]    # File name 
        folder_name = Ultilities.detach_current_ui(self.project_tab, JSON_PATH)[2]   # Folder name
    ...
    def slider_active(self, slider_handle):
        current_layout = Ultilities.detach_widget(self.project_tab)[2]
    ...
    def message_box(self, mess):
        ...
        if msgBox.clickedButton() == item_button:
            Ultilities.delete_item_selected(self.project_tab, JSON_PATH)
            self.reload()
